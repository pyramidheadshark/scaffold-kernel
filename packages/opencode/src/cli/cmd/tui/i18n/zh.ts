...
export const dict = {
  ...
  "tui.home.agreement.prefix": "使用 Scaffold 即表示你同意我们的",
  ...
  "tui.tips.free_api_sunset":
    "免费 API 服务已终止。请使用 {highlight}/login{/highlight} 登录，欢迎订阅Scaffold Token Plan或配置第三方 API 后使用 Scaffold。",
  "tui.tips.multi_skills": "在同一条消息中输入多个 {highlight}/skill-name{/highlight} 可以同时组合使用多个 Skills",
  "tui.tips.ask_slash_commands": "想找快捷指令？直接在聊天中问 {highlight}有哪些 slash 快捷指令？{/highlight}",
  ...
  "tui.tips.compose_next": "推荐前沿模型使用 {highlight}/compose-next{/highlight} 代替 Compose 智能体",
  ...
  "tui.tips.paste_image":
    "按 {highlight}Ctrl+V{/highlight} 把剪贴板中的图片粘贴到提示框（macOS 上请用 Ctrl+V，不要用 Cmd+V——Cmd+V 会被终端拦截）",
  ...
  "tui.tips.config_files":
    "创建 {highlight}scaffold.json{/highlight} 用于服务端配置，{highlight}tui.json{/highlight} 用于 TUI 配置",
  "tui.tips.global_config": "把 TUI 配置放在 {highlight}~/.config/scaffold/tui.json{/highlight} 作为全局配置",
  ...
  "tui.tips.mcp_oauth": "Scaffold 自动处理需要鉴权的远程 MCP 服务器的 OAuth",
  "tui.tips.custom_command":
    "在 {highlight}.scaffold/command/{/highlight} 添加 {highlight}.md{/highlight} 文件以定义可复用自定义提示",
  ...
  "tui.tips.custom_agent":
    "在 {highlight}.scaffold/agent/{/highlight} 添加 {highlight}.md{/highlight} 文件来创建专用 AI 角色",
  ...
  "tui.tips.formatter": "Scaffold 自动使用 prettier、gofmt、ruff 等格式化文件",
  ...
  "tui.tips.lsp": "Scaffold 使用 LSP 服务器进行智能代码分析",
  "tui.tips.custom_tool":
    "在 {highlight}.scaffold/tools/{/highlight} 创建 {highlight}.ts{/highlight} 文件定义新的 LLM 工具",
  "tui.tips.tool_scripts": "工具定义可以调用 Python、Go 等脚本",
  "tui.tips.plugins": "在 {highlight}.scaffold/plugin/{/highlight} 添加 {highlight}.ts{/highlight} 文件挂接事件钩子",
  "tui.tips.plugin_notify": "用插件在会话完成时发送系统通知",
  "tui.tips.plugin_protect": "写一个插件阻止 Scaffold 读取敏感文件",
  "tui.tips.run": "用 {highlight}scaffold run{/highlight} 进行非交互式脚本调用",
  "tui.tips.continue": "用 {highlight}scaffold --continue{/highlight} 继续上一个会话",
  "tui.tips.attach_cli": "用 {highlight}scaffold run -f file.ts{/highlight} 通过命令行附加文件",
  "tui.tips.format_json": "用 {highlight}--format json{/highlight} 在脚本中获得机器可读输出",
  "tui.tips.serve": "运行 scaffold serve 以无头模式提供 Scaffold API",
  "tui.tips.attach_server": "用 {highlight}scaffold run --attach{/highlight} 接入正在运行的服务",
  "tui.tips.upgrade": "运行 {highlight}scaffold upgrade{/highlight} 升级到最新版本",
  "tui.tips.auth_list": "运行 {highlight}scaffold auth list{/highlight} 查看所有已配置的 provider",
  "tui.tips.agent_create": "运行 {highlight}scaffold agent create{/highlight} 引导式创建智能体",
  "tui.tips.github_install": "运行 {highlight}scaffold github install{/highlight} 配置 GitHub workflow",
  ...
  "tui.tips.theme_files": "在 {highlight}.scaffold/themes/{/highlight} 目录下创建 JSON 主题文件",
  ...
  "tui.tips.debug_config": "运行 {highlight}scaffold debug config{/highlight} 排查配置问题",
  ...
  "tui.tips.zen": "搭配 Scaffold 使用 {highlight}/connect{/highlight} 选用经过测试的精选模型",
  ...
  "tui.skill.mimocode.description": "Scaffold 功能、配置与命令的自文档参考",
  ...
  "tui.skill.super-research.description":
    "自主研究——实验循环、主题调研、量化分析、对比评测、根因排查、消融实验、复现论文、写论文",
  ...
  "tui.skill.mimocode.slash": "Scaffold文档",
  ...
  "tui.context_limit.scalar_config":
    "compaction.max_context 当前是对所有模型生效的单值，请在 scaffold.json 中改为按模型配置",
  "tui.context_limit.shadowed": "已保存，但仍有其他配置将其固定为 {{value}}，请检查项目的 scaffold.json",
  ...
  "tui.dialog.free_api_sunset.message":
    "请使用 /login 登录，欢迎订阅Scaffold Token Plan或配置第三方 API 后使用 Scaffold。",
  ...
  "tui.model.mimo_auto.name": "Scaffold Auto（Scaffold-V2.5 限免至 7 月 26 日 18:00 · UTC+8）",
  ...
  "tui.dialog.try_best.handoff.description": "让 Scaffold 将未完成的工作委派给此 harness",
  ...
  "tui.toast.update_available.success": "已更新到 Scaffold v{{version}}，请重启应用。",
  ...
  "tui.voice.error.no_auth": "请先登录账号",
  ...
  "tui.dialog.login.flow.title": "Scaffold 登录",
  ...
  "trust.capabilities": "Scaffold 将能够读取、编辑和执行此目录中的文件。",
  ...
  "skip_permissions.body":
    "你使用 --dangerously-skip-permissions 启动。Scaffold 将在不征求同意的情况下读取、修改、执行文件并运行 Shell 命令。只有你在配置中明确拒绝（deny）的规则仍然生效。由此产生的一切后果由你自行承担。",
  "skip_permissions.plugin_warn":
    "在此模式下，恶意的提示词、文件或插件可以在无任何确认的情况下执行任意命令，并修改或窃取你的数据。",
  "skip_permissions.root_warn": "你正在以 root 身份运行。以 root 跳过权限确认会让模型对本机拥有不受限制的控制权。",
