...
async function stdinHasData(): Promise<boolean> {
  try {
    ...
    // ⚠ Здесь стоял внешний шелл с ключом ожидания у встроенного `read`, и он не работал
    //
    // НИ НА ОДНОЙ из двух сторон. Этот ключ — расширение bash/ksh/zsh; на Debian и Ubuntu
    // `/bin/sh` это `dash`, где его нет: шелл печатает «Illegal option», уходит в ветку
    // отказа, и проба говорит «данных нет» даже на трубе С ДАННЫМИ. То есть на самом
    // распространённом образе уведомление не печаталось никогда. А там, где `/bin/sh` это
    // bash, на ПУСТОЙ ЗАКРЫТОЙ трубе тот же вызов возвращает успех (EOF читается как
    // готовность) — и проба кричала ложно.
    //
    // ⚠ Дословную команду здесь не цитируем: гейт `track-r-no-shell-stdin-probe` ищет её
    // как запрещённый маркер, и объяснение стало бы нарушением. Тот же класс, что
    // «документирование отсутствия триггерит поиск».
    //
