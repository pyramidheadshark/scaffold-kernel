...
import { type HarnessMode, needsCompactGPTToolset, usesGPTToolset } from "@/tool/gpt"
...
export function provider(model: Provider.Model, harness?: HarnessMode) {
  ...
  const prompt = (id: string) => {
    if (needsCompactGPTToolset(id)) return PROMPT_GPT
  ...
  Effect.gen(function* () {
    ...
    return Service.of({
      environment: Effect.fn("SystemPrompt.environment")(function* (
        ...
        const base = [
          [
            `You are Scaffold, an interactive agent that helps users with software engineering tasks. Use the instructions below and the tools available to you to assist the user.`,
