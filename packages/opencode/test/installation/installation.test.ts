...
describe("installation", () => {
  describe("method", () => {
    test("detects npm when scaffold-kernel is in npm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "npm" && args.includes("-g")) return "scaffold-kernel@1.0.0"
          return ""
        },
      )
    ...
    test("detects pnpm when scaffold-kernel is in pnpm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "pnpm" && args.includes("-g")) return "scaffold-kernel@1.0.0"
          return ""
        },
      )
    ...
    test("detects bun when scaffold-kernel is in bun pm ls output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "bun" && args.includes("-g")) return "scaffold-kernel@1.0.0"
          return ""
        },
      )
    ...
    test("returns unknown when no package manager has scaffold-kernel", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        () => "",
      )
  ...
  describe("latest", () => {
    test("reads version from npm registry for npm method", async () => {
      const layer = testLayer(
        (req) => {
          expect(req.url).toContain(encodeURIComponent("scaffold-kernel"))
  ...
  describe("upgrade", () => {
    test("runs npm install with correct package", async () => {
      ...
      expect(capturedArgs).toContain("scaffold-kernel@2.0.0")
    })
